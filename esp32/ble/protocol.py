...
def encode_sonar_data(timestamp: int, distance_cm, baseline_cm,
                     status: int, fish_event: bool) -> bytes:
    """编码水下声呐实时帧。

    帧结构 (15 字节):
        CMD(1) + timestamp(4) + distance_cm(4 float) + baseline_cm(4 float) + status(1) + fish_event(1)

    Args:
        timestamp:    Unix 秒时间戳
        distance_cm:  当前距离（cm），可为 None
        baseline_cm:  近 N 秒滑动均值基线（cm），可为 None
        status:       0=正常 1=超出量程 2=过近 3=通讯失败
        fish_event:   True=检测到鱼经过
    """
