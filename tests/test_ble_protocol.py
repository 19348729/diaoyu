...
def js_decode_realtime(data: bytes) -> dict:
    ...
    tw_raw = struct.unpack_from("<f", data, 5)[0]
    ta_raw = struct.unpack_from("<f", data, 9)[0]
    pl_raw = struct.unpack_from("<f", data, 13)[0]

    tw = _fmt_temp(_to_nullable(tw_raw, TEMP_NONE))
    ta = _fmt_temp(_to_nullable(ta_raw, TEMP_NONE))
...
def js_decode_history_batch(data: bytes) -> dict:
    ...
    for i in range(count):
        ...
        tw_raw = struct.unpack_from("<f", data, offset + 4)[0]
        ta_raw = struct.unpack_from("<f", data, offset + 8)[0]
        pl_raw = struct.unpack_from("<f", data, offset + 12)[0]
...
class TestFrameEncoding:
    ...
    def test_realtime_roundtrip_normal(self):
        ...
        assert abs(decoded["tWater"] - tw) < 0.01
        assert abs(decoded["tAir"] - ta) < 0.01
        assert abs(decoded["pLocal"] - pl) < 0.01

    # B1-3
    def test_realtime_with_none(self):
        frame = encode_realtime_data(1714000000, None, 20.0, None)
        decoded = js_decode_realtime(frame)
        assert decoded["tWater"] is None
        assert abs(decoded["tAir"] - 20.0) < 0.01
        assert decoded["pLocal"] is None

    # B1-4
    def test_realtime_tdiff(self):
        pass # Removed as tDiff is no longer in V2

    # B1-5
    def test_history_batch_3_records(self):
        records = [
            (1714000000 + i * 5, 18.0 + i * 0.1, 19.0, 1008.0)
            for i in range(3)
        ]
        frame = encode_history_batch(records)
        assert len(frame) == 3 + 3 * 16  # 51 bytes
        decoded = js_decode_history_batch(frame)
        assert decoded["count"] == 3
        assert len(decoded["records"]) == 3
        assert decoded["records"][0]["timestamp"] == 1714000000
        assert abs(decoded["records"][2]["tWater"] - 18.2) < 0.01
...
class TestProtocolEdgeCases:
    ...
    def test_temp_placeholder_precision(self):
        frame = encode_realtime_data(1000, None, None, 1008.0)
        decoded = js_decode_realtime(frame)
        assert decoded["tWater"] is None
        assert decoded["tAir"] is None

    # B2-6 气压占位符
    def test_pressure_placeholder(self):
        frame = encode_realtime_data(1000, 18.0, 19.0, None)
        decoded = js_decode_realtime(frame)
        assert decoded["pLocal"] is None
    ...
    def test_float_precision(self):
        val = 18.123456789
        frame = encode_realtime_data(1000, val, 19.0, 1008.0)
        decoded = js_decode_realtime(frame)
        # 32位float精度约6-7位有效数字，round(2)后误差在0.01内
        assert abs(decoded["tWater"] - val) < 0.01
...
class TestRingBuffer:
    ...
    def test_write_10(self):
        ...
        for i in range(10):
            buf.write(1000 + i * 5, 18.0, 19.0, 1008.0)
        assert buf.count == 10
    ...
    def test_overflow(self):
        ...
        for i in range(8):
            buf.write(1000 + i * 5, float(i), 0.0, 0.0)
        assert buf.count == 5
    ...
    def test_get_status(self):
        buf = RingBuffer(capacity=100)
        for i in range(10):
            buf.write(1000 + i * 5, 0.0, 0.0, 0.0)
        buf.mark_sent(3)
